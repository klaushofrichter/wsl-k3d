...
echo
echo "==== $0: Generating ${CALLS} API calls to http://localhost:${HTTPPORT} with between ${DELAY}s and $( bc <<< "scale=2; ${DELAY} + ${SPREAD}" )s delay between the calls."
i="1"
while [ true ] 
do
  TOTALDELAY=$( bc <<< "scale=2; ${DELAY} + ${SPREAD} * $RANDOM / 32767" ) 
  RES="$0: $(date '+%H:%M:%S') ${i}/${CALLS}: delay: ${TOTALDELAY}s "
  if [ $(( $RANDOM % 2 )) == 0 ]; then
    RES="${RES} $(curl -s http://localhost:${HTTPPORT}/service/info)"
  else
    RES="${RES} $(curl -s http://localhost:${HTTPPORT}/service/random)"
  fi
  if [ ${#RES} -gt ${COLS} ]; then
    echo "${RES:0:${COLS}}..."
  else
